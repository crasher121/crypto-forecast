...
MODELS_DIR = "/models"
N_LAGS = 7                       # сколько прошлых остатков подаем в лес
SARIMA_ORDER = (1, 1, 1)
SARIMA_SEASONAL = (1, 0, 1, 7)   # недельная сезонность по дням


# тянем дневной ряд (часовые цены -> close по дням)
...
# прогноз на horizon_days дней вперед
def make_forecast(symbol, horizon_days, store):
    ...
    ci = np.array(fc.conf_int(alpha=0.2))   # 80 perc интервал
    lower = ci[:, 0]
    upper = ci[:, 1]

    # лес добавляет прогноз остатков рекурсивно
    rf = bundle["rf"]
    used = bundle["used_rf"]
